...
function applyAdCleanup(parsedResponse, adBlockEnabled) {
  if (!adBlockEnabled) return;

  if (parsedResponse.adPlacements) parsedResponse.adPlacements = [];
  if (parsedResponse.playerAds) parsedResponse.playerAds = false;
  if (parsedResponse.adSlots) parsedResponse.adSlots = [];

  if (!Array.isArray(parsedResponse) && parsedResponse?.entries) {
...
function applyBrowseAdFiltering(parsedResponse, adBlockEnabled) {
  ...
  if (!Array.isArray(shelves) || !adBlockEnabled) return;
  ...
  for (const shelf of parsedResponse.contents.tvBrowseRenderer.content.tvSurfaceContentRenderer.content.sectionListRenderer.contents) {
    const items = shelf?.shelfRenderer?.content?.horizontalListRenderer?.items;
    if (!Array.isArray(items)) continue;

    shelf.shelfRenderer.content.horizontalListRenderer.items = items.filter((item) => !item.adSlotRenderer);
  }
}

function applyShortsAdFiltering(parsedResponse, adBlockEnabled) {
  if (Array.isArray(parsedResponse) || !adBlockEnabled || !parsedResponse?.entries) return;
...
function processShelves(shelves) {  
  ...
  for (let i = shelves.length - 1; i >= 0; i--) {
    try {
      ...
      // Shorts shelf removal by title is handled by local adblock helpers.
      if (!shortsEnabled) {
...
function getCurrentPage() {
  ...
  else if (browseParamPageMap[browseParam]) {
    detectedPage = browseParamPageMap[browseParam];
  }
